...
CBody::CBody(b2World* _world, sf::Vector2f _position, sf::Vector2f _size, b2BodyType _type)
{
    ...
    if (!m_texture->loadFromFile("Resources/Rect.png", sf::IntRect(0, 0, 100, 100))) {
        std::cerr << "\nWARNING: <CBody::CBody> \"Resources/Rect.png\" does not exist.\n";
    }
...
CBody::CBody(b2World* _world, sf::Vector2f _position, float _radius, b2BodyType _type)
{
    m_sprite = new sf::Sprite();
    (m_sprite)->setPosition(_position);
    m_texture = new sf::Texture();
    if (!m_texture->loadFromFile("Resources/Circle.png", sf::IntRect(0, 0, 100, 100))) {
        std::cerr << "\nWARNING: <CBody::CBody> \"Resources/Circle.png\" does not exist.\n";
    }
    (m_sprite)->setTexture(*m_texture);
    (m_sprite)->setOrigin(m_sprite->getGlobalBounds().width / 2, m_sprite->getGlobalBounds().height / 2);
